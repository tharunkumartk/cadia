...
const getProof = async (wallet_address: string, captcha_text: string) => {
  ...
  console.log(wallet_address, captcha_text);
  const resp = await fetch(proverAPI, {
    method: "POST",
...
const submitCaptcha = async (
  account: string,
  entry: string,
  setProofState: React.Dispatch<React.SetStateAction<Buffer>>,
) => {
...
const CaptchaDialog = ({ open, handleClose, closeCaptcha, account, setProofState }: CaptchaDialogProps) => {
        ...
        >
          <Grid container sx={{ marginTop: "2vh", justifyContent: "center", alignItems: "center" }}>
            ...
            <SubmitCaptchaButton
              text="Submit"
              onClick={() => {
                submitCaptcha(account, entry, setProofState);
                handleClose();
              }}
            />
          </Grid>
